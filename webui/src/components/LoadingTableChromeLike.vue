  ...
  <v-card class="ma-4">
    ...
    >
      ...
      <template v-slot:item="{ item }">
        <tr
          :class="`text-no-wrap text-${
            item.http_status < 400 ? 'body' : 'red'
          }`"
        >
          <td>
            <v-avatar size="32">
          ...
          <td class="d-flex justify-center align-center">
            <div
              ...
              :style="`background-color: ${getColor(
                item.http_status
              )}; border-radius: 4px; width: 40px; color: #333333;
`"
            >
              {{ item.http_status }}
            </div>
      ...
      <template v-slot:item.http_status="{ value }">
        <div
          ...
          :style="`background-color: ${getColor(
            value
          )}; border-radius: 180px; max-width: 40px; color: black;
font-weight: bold;
`"
        >
          {{ value }}
...
function getColor(status: string): string {
  ...
  else if (200 <= n && n <= 299) return "#86de74";
